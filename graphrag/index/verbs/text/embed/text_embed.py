...
from graphrag.llm.openai.embedding_llm import load_strategy, TextEmbedStrategyType
...
async def text_embed(
    ...
    **kwargs,
    ...
    ```yaml
    strategy:
        type: openai, # The type of strategy to use for embedding available options are: openai, azure_openai, ollama, vllm, sentence_transformer
        llm: # The configuration for the LLM
            type: openai_embedding # the type of llm to use, available options are: openai_embedding, azure_openai_embedding, ollama, vllm, sentence_transformer
...
async def _text_embed_in_memory(
    ...
    log.info(f"Embedding text in column {column} using strategy {strategy}")    
